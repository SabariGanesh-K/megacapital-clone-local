...
export default function Blog() {
  ...
  const btnStyle1 = { color:"#56C5FF", backgroundColor:"#2E2F30", width:"85px", border:"none", height:"40px" }
  const btnStyle2 = { color:"white", backgroundColor:"#24B6E6", width:"85px", border:"none", height:"40px" }
      ...
      {/* <Container maxWidth='md'> */}
            <MHidden width="mdDown">
              <Grid paddingLeft={'11%'} paddingRight={'11%'}>
                  ...
                  <Box component="h1" class="text-info">News & Blog</Box>
                  </Grid>
                    ...
                    {blogsList.map((item, key) => (
                      <Grid item md="4">
                        <Grid container direction="column" sx={{width:"100%"}}>
                          <Box width="100%" component="a" href={`/blog/${item.slug}`} sx={{ textDecoration: 'none' }}>
                            <Box component="img" width="100%" src={item.image} borderRadius={1}></Box>
                            <Box color="#00BFFF" marginTop="10px"> {moment(item.createdAt).format('MMMM D, YYYY')} </Box>
                            <Box marginTop="10px"  color="white"> {item.title}  </Box>
                          </Box>
                        </Grid>
                      </Grid>
                    ))}
                  ...
                  <Grid container spacing={2}>
                    {newsList.map((item, key) => (
                      <Grid item md="4">
                        <Grid container direction="column" sx={{width:"100%"}}>
                          <Box width="100%" component="a" href={`/news/${item.slug}`} sx={{ textDecoration: 'none' }}>
                            <Box component="img" width="100%" src={item.image} borderRadius={1}></Box>
                            <Box color="#00BFFF" marginTop="10px"> {moment(item.createdAt).format('MMMM D, YYYY')} </Box>
                            <Box marginTop="10px"  color="white"> {item.title}  </Box>
                          </Box>
            ...
            <MHidden width="mdUp">
              <Grid paddingLeft={'5%'} paddingRight={'5%'}>
                  ...
                  <Grid width="100%" display="flex" justifyContent="center" marginBottom="25px" marginTop="25px">
                      <Box component="button" borderRadius={0.5} style={checked === 1 ? btnStyle1 : btnStyle2} onClick={() => setCheck(0)}>Blog</Box>
                      <Box component="button" borderRadius={0.5} style={checked === 1 ? btnStyle2 : btnStyle1} onClick={() => setCheck(1)}>News</Box>
                  </Grid>
                  {checked === 0 ?
                  <Grid container spacing={2}>
                    {blogsList.map((item, key) => (
