    ...
    return (
      <Page title="Blog List">
        <Container maxWidth="lg" className="pt-5">
          ...
          >
            <Typography variant="h4">Blog List
            <Box
                component='button'
                borderRadius={1}
                padding='10px 5px'
            ...
            <Stack sx={{ mt: 2 }} spacing={3}>
                <Table>
                    ...
                    <TableBody>
                    {BlogLists.map((item, key) => (
                        <TableRow key={key}>
                            <TableCell>{key+1}</TableCell>
                            <TableCell>{item.title}</TableCell>
                            <TableCell><img src={item.image} alt={item.slug} width="20%" /></TableCell>
                            <TableCell><a href={`/editBlog/${item.slug}`}>Edit</a>&nbsp;<a onClick={(event) => deleteBlog(event, item._id)}>Delete</a></TableCell>
                        </TableRow>
                    ))}
                    </TableBody>
                </Table>
